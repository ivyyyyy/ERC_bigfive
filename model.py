...
class DialogueRNNCell(nn.Module):
    ...
    def __init__(self, D_m, D_g, D_p, D_e, listener_state=False,
        ...
        self.g_cell = nn.GRUCell(D_m+D_p,D_g) # 100+500, 500
        self.p_cell = nn.GRUCell(D_m+D_g,D_p) # 100+500, 500
        self.e_cell = nn.GRUCell(D_p,D_e) # 500, 300
        if listener_state:
    ...
    def forward(self, U, qmask, g_hist, q0, e0):
        ...
        e0 -> batch, self.D_e
        """
        qm_idx = torch.argmax(qmask, 1)  # batch
        q0_sel = self._select_parties(q0, qm_idx) # batch, self.D_p

        g_ = self.g_cell(torch.cat([U,q0_sel], dim=1), # batch, self.D_g
        ...
        U_c_ = torch.cat([U,c_], dim=1).unsqueeze(1).expand(-1,qmask.size()[1],-1) # batch, party, self.D_g+self.D_m
...
class BiModel(nn.Module):
    ...
    def forward(self, U, qmask, umask, bigfive, label,att2=True):
        """
        U -> seq_len, batch, D_m
        qmask -> seq_len, batch, party
        """
        ...
        log_prob = log_prob + self.bigfivel(bigfive)
        loss = -self.crf(log_prob, label, umask.T.byte())

        if att2:
            return log_prob, alpha, alpha_f, alpha_b, loss
        else:
